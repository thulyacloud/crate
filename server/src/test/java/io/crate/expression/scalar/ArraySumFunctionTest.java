...
public class ArraySumFunctionTest extends ScalarTestCase {
    ...
    public void test_array_returns_sum_of_elements() {
        ...
        for(DataType type: typesToTest) {
            ...
            var expected = inputDependantOutputType.implicitCast(valuesToTest.stream()
            ...
            String expression = String.format("array_sum(?::%s[])", type.getName());
    ...
    public void test_array_big_numbers_casting_to_numeric_returns_sum() {
        assertEvaluate("array_sum(cast([9223372036854775807,9223372036854775807] as array(numeric)))",
    ...
    @Test
    public void test_all_elements_nulls_results_in_zero() {
        assertEvaluate("array_sum(cast([null, null] as array(integer)))", 0L);
    }

    @Test
    public void test_null_array_results_in_zero() {
        assertEvaluate("array_sum(null::int[])", 0L);
    }

    @Test
    public void test_null_array_given_directly_results_in_zero() {
        assertEvaluate("array_sum(null)", 0L);
    }

    @Test
    public void test_empty_array_results_in_zero() {
        assertEvaluate("array_sum(cast([] as array(integer)))", 0L);
    }

    @Test
    public void test_empty_array_given_directly_throws_exception() {
        assertThrows(() -> assertEvaluate("array_sum([])", 0L),
